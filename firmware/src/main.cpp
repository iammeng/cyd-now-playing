...
volatile int pendingCmd = 0; // 1=playpause 2=next 3=prev 4=shuffle 5=repeat 6=seek 7=volume
volatile long pendingArg = 0;
SemaphoreHandle_t stateMux;

enum Mode { BOOT, PLAYING, IDLE, OFFLINE };
Mode mode = BOOT;
...
void drawStatusDot() {
  uint16_t c = mode == PLAYING ? C_BGNOW : C_BG;
...
void dispatchTap(int x, int y) {
  ...
  // art / text area: no action (avoids accidental skips)
}
...
void loop() {
  ...
  trackDirty = playDirty = artDirty = false;
  ...
  if (now - lastTickMs >= 1000) {
    ...
    drawStatusDot();
    updateBacklight();
    if (lastDataMs && now - lastDataMs > STALE_REBOOT_MS) ESP.restart();
  }
